...
layout(binding=0) uniform vs_scene_default_params {
    mat4 u_model;
	mat4 u_mvp;
};
...
layout(binding=0) uniform texture2D scene_default_tex;
layout(binding=0) uniform sampler scene_default_smp;

layout(binding=1) uniform fs_scene_default_params {
	vec4 u_tint;
	vec2 u_tl;
	vec2 u_br;
	vec3 u_view_pos;
...
void main() {
	...
	//srgb->linear
	//vec3 base_col_srgb=texture(sampler2D(scene_default_tex, scene_default_smp),u_tl + uv * (u_br - u_tl)).rgb;
	vec3 base_col_srgb=texture(sampler2D(scene_default_tex, scene_default_smp), uv).rgb;
...
@program scene_default vs_scene_default fs_scene_default
...
layout(binding = 0) uniform vs_scene_line_params {
	mat4 u_mvp;
};
...
layout(binding = 1) uniform fs_scene_line_params{
	vec4 u_tint;
};
...
@program scene_line vs_scene_line fs_scene_line
